...
def _subplot_grid(n, ncols=3, **kwargs):
    """A (nrows, ncols) grid sized to fit n panels."""
...
# Try to load an existing MrVI model for age analysis, or train one if none exists
def try_load_mrvi_model(adata):
    ...
    try:
        return MRVI.load(str(AGE_MODEL_DIR), adata=adata)
...
# Instead of coloring by subtype, color by age group (Infant, Early Childhood, Late Childhood, Adolescence)
def plot_number_age_groups(fractions, output_dir):
    ...
    nrows = math.ceil(len(SUBTYPE_ORDER) / 3)
    fig, axes = _subplot_grid(len(SUBTYPE_ORDER), figsize=(11, 3.4 * nrows), sharey=False)
    for ax, subtype in zip(axes, SUBTYPE_ORDER):
        ...
        for patch, group in zip(box["boxes"], groups):
            patch.set_facecolor(palette[group]) # set box color per age group
            patch.set_alpha(0.6)
...
# Points are colored by age group instead of per-subtype color
def plot_number_continuous_age(fractions, output_dir):
    ...
    for panel_idx, (ax, subtype) in enumerate(zip(axes, SUBTYPE_ORDER)):
        sub = fractions[fractions["subtype"] == subtype].dropna(subset=[AGE_KEY, "fraction"])
        for group, color in palette.items():
            grp = sub[sub[AGE_GROUP_KEY].astype(str) == group]
            ax.scatter(
                grp[AGE_KEY], grp["fraction"], s=22, color=color, alpha=0.85, linewidth=0,
                label=group if panel_idx == 0 else None,
            )
